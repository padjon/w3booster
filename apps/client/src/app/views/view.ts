...
export abstract class View implements OnInit, OnDestroy, AfterViewInit {
  ...
  */
  ...
  public openModal(content) {
    this.injector.get(NgbModal).open(content, { backdropClass: 'ce-backdrop' }).result.then((result) => {
    });
  }
  ...
  public getLocalizedDate(timestamp: number | Date) {
    return new Date(timestamp).toLocaleDateString(undefined, { month: 'short', day: '2-digit', year: 'numeric'});
  }
