...
//ORIGINAL LINE: [StructLayout(LayoutKind.Sequential, Pack = 1)] public unsafe struct HeroInfo
...
//
//		public const int SIZE = sizeof(uint) + MAX_NAME_LENGTH * sizeof(byte) + MAX_BUTTON_ART_LENGTH * sizeof(byte) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + sizeof(uint) + MAX_ABILITIES * AbilityInfo.SIZE + sizeof(uint) + MAX_ITEMS * ItemInfo.SIZE;
//
...
//
//		[MarshalAs(UnmanagedType.ByValTStr, SizeConst = MAX_ABILITIES * AbilityInfo.SIZE)] private fixed byte abilities[MAX_ABILITIES * AbilityInfo.SIZE];
//
//		private readonly uint itemCount;
//
//		[MarshalAs(UnmanagedType.ByValTStr, SizeConst = MAX_ITEMS * ItemInfo.SIZE)] private fixed byte items[MAX_ITEMS * ItemInfo.SIZE];
//
//		public string Name
//		{
//			get
