...
myHostInfo::myHostInfo()
{
	...
	try 
	{
		...
		if (hostPtr == NULL)
		{
			...
			exception* gethostbynameException = new exception(errorMsg.c_str(), errorCode);
...
myHostInfo::myHostInfo(string& hostName,hostType type)
{
	...
	try 
	{
		...
		else if (type == ADDRESS)
		{
			...
			if (netAddr == -1)
			{
				exception* inet_addrException = new exception("Error calling inet_addr()", 0);
			...
			if (hostPtr == NULL)
			{
					...
					exception* gethostbyaddrException = new exception(errorMsg.c_str(), errorCode);
		...
		{
			exception* unknownTypeException = new exception("unknown host type: host name/address has to be given", 0);
