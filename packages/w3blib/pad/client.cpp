...
mySocket::mySocket(int pNumber)
{
    ...
    try
    {
		if ( (socketId=socket(AF_INET,SOCK_STREAM,0)) == -1)
        {
				...
				exception* openWinSocketException = new exception(errorMsg.c_str(), errorCode);
				throw openWinSocketException;
			#endif

			#ifdef UNIX
				std::exception* openUnixSocketException = new std::exception(0,"unix: error getting host by name");
...
void mySocket::setDebug(int debugToggle)
{
	try 
	{
		if ( setsockopt(socketId,SOL_SOCKET,SO_DEBUG,(char *)&debugToggle,sizeof(debugToggle)) == -1 )
		{
				...
				exception* socketOptionException = new exception(errorMsg.c_str(), errorCode);
				throw socketOptionException;
			#endif

			#ifdef UNIX
				std::exception* unixSocketOptionException = new std::exception(0,"unix: error getting host by name");
...
void mySocket::setKeepAlive(int aliveToggle)
{
	try 
	{
		if ( setsockopt(socketId,SOL_SOCKET,SO_KEEPALIVE,(char *)&aliveToggle,sizeof(aliveToggle)) == -1 )
		{
				...
				detectErrorSetSocketOption(&errorCode,errorMsg);
				std::exception* socketOptionException = new std::exception(errorMsg.c_str());
				throw socketOptionException;
			#endif
...
void myTcpSocket::bindSocket()
{
	try
	{
		if (bind(socketId,(struct sockaddr *)&clientAddr,sizeof(struct sockaddr_in))==-1)
		{
				...
				std::exception* socketBindException = new std::exception(errorMsg.c_str());
				throw socketBindException;
			#endif
			#ifdef UNIX
				std::exception* unixSocketBindException = new std::exception(0,"unix: error calling bind()");
	...
	*/
	...
	try 
	{
		if (connect(socketId,(struct sockaddr *)&serverAddress,sizeof(serverAddress)) == -1)
		{
				...
				std::exception* socketConnectException = new std::exception(errorMsg.c_str());
				throw socketConnectException;
			#endif

			#ifdef UNIX
				std::exception* unixSocketConnectException = new std::exception(0,"unix: error calling connect()");
...
myTcpSocket* myTcpSocket::acceptClient(string& clientHost)
{
	...
	try 
	{
		if ((newSocket = accept(socketId, (struct sockaddr *)&clientAddress,&clientAddressLen)) == -1)
		{
				...
				std::exception* socketAcceptException = new std::exception(errorMsg.c_str());
				throw socketAcceptException;
			#endif

			#ifdef UNIX
				std::exception* unixSocketAcceptException = new std::exception(0,"unix: error calling accept()");
...
void myTcpSocket::listenToClient(int totalNumPorts)
{
	try 
	{
		if (listen(socketId,totalNumPorts) == -1)
		{
				...
				std::exception* socketListenException = new std::exception(errorMsg.c_str());
				throw socketListenException;
			#endif

			#ifdef UNIX
				std::exception* unixSocketListenException = new std::exception(0,"unix: error calling listen()");
...
int myTcpSocket::sendMessage(string& message)
{
	...
	try 
	{
		if (numBytes = send(socketId,sendMsg.c_str(),sendMsg.size(),0) == -1)
		{
				...
				std::exception* socketSendException = new std::exception(errorMsg.c_str());
				throw socketSendException;
			#endif

			#ifdef UNIX
				std::exception* unixSocketSendException = new std::exception(0,"unix: error calling send()");
...
int myTcpSocket::sendMessage(string& message)
{
	...
	*/
	...
	BYTE* pointer = (BYTE*)&size;
	msgLength[0] = pointer[1];
	msgLength[1] = pointer[0];
	string sendMsg = string(msgLength, 2);
    sendMsg += message;
	...
	try
	{
		while ( numBytes < currentSize )
		{
			...
			if (numBytes == -1)
			{
				...
				std::exception* socketRecvException = new std::exception(errorMsg.c_str());
	...
	try
	{
		while ( received < msgSize )
		{
			...
			if (numBytes == -1)
			{
				...
				string errorMsg = "error calling recv():\n";
				detectErrorRecv(&errorCode,errorMsg);
				std::exception* socketRecvException = new std::exception(std::to_string(errorCode).c_str(),errorCode);
				throw socketRecvException;
			}
...
int myTcpSocket::recieveMessage(string& message)
{
	...
	try
	{
        ...
        if (numBytes == -1)
        {
			std::exception* unixSocketRecvException = new std::exception("unix: error calling recv()", 0);
